...
Key dependencies imported here indicate coupling points: next/server.

Primary exported entry points: isAppError, toErrorResponse, assert.
...
This file currently has approximately 48 lines, which is manageable but should still be monitored for responsibility creep.
