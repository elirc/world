...
Key dependencies imported here indicate coupling points: next/headers, jose, @/lib/db, @/lib/errors.

Primary exported entry points: createSessionToken, setSession, clearSession, readSessionFromRequest, readSessionFromCookies, normalizeUserContext, getRequestUser, getServerUser, hasRole, hasPermission, requirePermission, requireAuth.
...
This file currently has approximately 175 lines, which is manageable but should still be monitored for responsibility creep.
