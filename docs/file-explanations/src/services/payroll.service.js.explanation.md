...
Key dependencies imported here indicate coupling points: node:fs, node:path, pdfkit, @/lib/db, @/lib/errors, @/lib/money, @/lib/audit, @/lib/events, @/services/context, @/services/notification.service.

Primary exported entry points: listPayrollRuns, getPayrollRun, listPayrollItems, createPayrollRun, calculatePayrollRun, approvePayrollRun, processPayrollRun.
...
Cross-cutting concerns currently visible in this file include: transactional consistency, audit logging, domain event emission, notification dispatch, money math using minor units.
...
This file currently has approximately 656 lines, which is manageable but should still be monitored for responsibility creep.
